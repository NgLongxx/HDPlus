...
$(document).on('submit', '#form-recha', function(event) {
    event.preventDefault();
    error.remove();
    var btn = $(this).children('.button');
    var data = new FormData(this);
    ...
    $.ajax({
        ...
        success: function(res) {
            btn.attr('class', 'button');
            error.text(res.success).appendTo('#result_recha');
        },
        error: function(err) {
            btn.attr('class', 'button');
            error.text(err.responseJSON.message).appendTo('#result_recha');
        }
    });
});
$(document).on('click', '#credit-user', function(e) {
    var id = $(this).data('id');
    $.ajax({
        url: homeUrl + '/admin/user/credit',
        data: { 'id': id },
        success: function(res) {
            $('<div/>').addClass('over').appendTo('body');
