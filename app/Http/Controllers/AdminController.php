...
class AdminController extends Controller
{
    ...
    public function film(Request $request, $action = null){
        switch($action){
                ...
                if($request->method() === "POST"){
                    if(empty($request->name) || empty($request->about) || empty($request->actor) || empty($request->director) || empty($request->episode) || empty($request->category_parent)){
                        ...
                        if($request->hasFile('poster')){
                            ...
                            $coverName = md5(time()).$request->cover->getClientOriginalExtension();
                            if($request->poster->move(public_path('upload/posters'), $posterName)){
                                $film->poster = url('upload/posters/'.$posterName);    
                                                 
                            
                            if($request->cover->move(public_path('upload/covers'), $coverName)){
                                $film->cover = url('upload/covers/'.$coverName);    
                            ...
                            if(empty($request->cover)){
                                return response()->json(['message' => 'Vui lòng điền cover phim'], 422);
                            }
            ...
            break;
            default:
                $film = Film::orderBy('id', 'desc')->paginate(6);
            ...
            case 'credit':
                $user = User::findOrFail($request->id);
                 if($request->method() === "POST"){
                    ...
                    if($user){
                        $html = "<center><div class='alert alert-success'>
                        Đã nạp <b>$request->credit</b> cho thành viên <b>$user->email</b> thành công. Tải lại trang để có hiệu lực!
                        </div>
                        </center>";
                        return view('dialog', ['html' => $html]);
                 }else {
                    $html = "<center><div class='alert alert-danger'>
                         Lỗi nạp!! Vui lòng thử lại
                        </div>
                        </center>";
                        return view('dialog', ['html' => $html]); 
                 }}
            return view('admin/user/credit');
            break;
    ...
    public function report(Request $request){
        ...
        $report = Report::orderBy('id', 'desc')->paginate(10);
        $film= Film::all();
        return view('admin.report.reports', ['report' => $report, 'film'=> $film]);
